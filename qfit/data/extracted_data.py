...
class ActiveExtractedData(QAbstractTableModel):
    ...
    def insertColumn(self, column: QModelIndex, parent=QModelIndex(), *args, **kwargs):
        ...
        self._data = np.insert(self._data, column, np.asarray([0.0, 0.0]),
                               #"None"]),
                               axis=1)
...
class AllExtractedData(
    ...
    def removeMatchingData(self, column: QModelIndex):
        self.assocDataList[0] = np.delete(self.assocDataList[0], column, axis=1)
        self.layoutChanged.emit()
        return True
